...
bool Cell::WouldIntroduceCircularDependency(const Impl& new_impl) const {
    ...
    std::unordered_set<const Cell*> reference;
    for (const Position& pos : new_impl.GetReferencedCells()) {
        reference.insert(sheet_.GetCellPtr(pos));
    }

    std::unordered_set<const Cell*> visit;
    ...
    while (!to_visit.empty()) {
        ...
        if (reference.find(current) != reference.end()) { return true; }

        for (const Cell* incoming : current->l_nodes_) {
            if (visit.find(incoming) == visit.end()) to_visit.push(incoming);
        }
    }
...
void Cell::Set(std::string text) {
    ...
    if (text.empty()) { 
        ...
        if (text[0] == FORMULA_SIGN && text.size() > 1) { impl = std::make_unique<FormulaImpl>(std::move(text), sheet_); }
    ...
    for (Cell* out_going : r_nodes_) {
        out_going->l_nodes_.erase(this);
    }

    r_nodes_.clear();

    for (const Position& pos : impl_->GetReferencedCells()) {
        Cell* out_going = sheet_.GetCellPtr(pos);
        if (!out_going) {
            sheet_.SetCell(pos, "");
            out_going = sheet_.GetCellPtr(pos);
        }
        r_nodes_.insert(out_going);
        out_going->l_nodes_.insert(this);
    }
